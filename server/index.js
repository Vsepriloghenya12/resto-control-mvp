...
function lineIsTtkNoise(line) {
  ...
  if (/^(№|No|Nº|N\s|Наименование продукта|Ед\.?\s*изм|Брутто|Вес брутто|Вес нетто|Вес готового|Технология приготовления)/i.test(text)) return true;
  if (/(наименование продукта|ед\.?\s*изм|брутто в ед|вес брутто|вес нетто|вес готового|технология приготовления)/i.test(text)) return true;
  if (/^(п\/?ф,?\s*)?(кг|г|л|мл|шт\.?|порц\.?)\s+продукта,?\s*(кг|г|л|мл|шт\.?|порц\.?)$/i.test(text)) return true;
